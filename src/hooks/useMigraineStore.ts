...
function loadFromStorage(): MigraineEpisode[] {
  try {
...
function saveToStorage(episodes: MigraineEpisode[]) {
  localStorage.setItem(STORAGE_KEY, JSON.stringify(episodes));
}

export function useMigraineStore() {
  const [episodes, setEpisodes] = useState<MigraineEpisode[]>(() => loadFromStorage());
